pipeline {
    ...
    stages {
        ...
        stage("Build Docker Image") {
            steps {
                script {
                    env.IMAGE_TAG = "mithiranaws/amazon:${BUILD_NUMBER}"
            ...
            steps {
                script {
                    withCredentials([string(credentialsId: 'docker-cred', variable: 'dockerpwd')]) {
                        sh "docker login -u mithiranaws -p ${dockerpwd}"
                        sh "docker tag amazon ${env.IMAGE_TAG}"
                        sh "docker push ${env.IMAGE_TAG}"

                        // Also push latest
                        sh "docker tag amazon mithiranaws/amazon:latest"
                        sh "docker push mithiranaws/amazon:latest"
                    }
                }
            }
        }
